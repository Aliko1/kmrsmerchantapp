...
document.addEventListener("pageinit", function(e) {
	...
	var check_val = $("#check_internal_timer").val();
	
	if( check_val > 0 ) { 
	 $("#old_internal_timer").val(check_val);
	}
	
	//$("#check_internal_timer").val(0);
           ...
           ajax_request.abort();
			...
			switch(action)
			{
				...
				default:
				   ...
				   if( action == 'GetTodaysOrder' || action == 'GetTodaysOrder' || action == 'GetPendingOrders' ) { 
				   
					  if( $("#check_internal_timer").val() == 1 || $("#check_internal_timer").val() == 2 || $("#check_internal_timer").val() == 3) { 
						 kloader.show();
					  } else { 
	...
	success: function (data) {	  
		...
		if (data.code==1){
			...
			switch (action)
			{
				...
				case "OtherList":
				otherList(data.details);
				break;	
				
				case "OutOfStock":
				outOfStockList(data.details);
				break;					
...
function callAjax_Timer(action,params)
{	
...
function GetTodaysOrder1()
{	
	var info=getMerchantInfoStorage();	
	var params="token="+getStorage("merchant_token");
	params+="&user_type="+info.user_type;
	params+="&mtid="+info.merchant_id;
	callAjax_Timer("GetTodaysOrder",params);
}
...
function getPendingOrders1()
{	
	var info=getMerchantInfoStorage();	
	var params="token="+getStorage("merchant_token");
	params+="&user_type="+info.user_type;
	params+="&mtid="+info.merchant_id;
	callAjax_Timer("GetPendingOrders",params);
}
...
function getGetAllOrders1()
{	
	var info=getMerchantInfoStorage();	
	var params="token="+getStorage("merchant_token");
	params+="&user_type="+info.user_type;
	params+="&mtid="+info.merchant_id;
	callAjax_Timer("GetAllOrders",params);
}
...
function displayOrders(data,div_id)
{
	...
	var htm='<div class="new_margin"></div>';
	...
	$.each( data, function( key, val ) {  
		...
		if( i%2 != 0 ) { 
		   new_cls = 'list_back_color';
		}else { 
		   new_cls = '';
		}
						   
		htm+= '<div class="my_class11 '+new_cls+'">';
		htm+='<ons-list-item modifier="tappable" onclick="viewOrder('+val.order_id+');" class="my_class">';
		...
		htm+='<ons-col class="first_text" >';
		   htm+='<p class="margin2 small-font-dim small-font-dim-smaller">ordernr: '+val.order_id+'</p>';
		   htm+='<p class="margin2 small-font-dim small-font-dim-smaller">order placed at: '+val.date_created+'</p>';
		   htm+='<br class="clear" >';
		   htm += '<p class="small-font-dim orange-text jat">customer expects on: '+val.delivery_time+'</p>';
		   htm+='</ons-col>';
		   
		   htm += '<ons-col class="text-right">';
		   htm+='<p class="status margin2 small-font-dim small-font-dim-smaller">Status:';
		   htm+="<span style='font-weight:300;'></span> "+val.status;
		   htm+='</p>';
		   htm+='</ons-col>';
		 
		 
           htm+='<ons-col  class="text-middle" style="float:right; text-align:right;">';
		   htm+='<price style="float:right; text-align:right;">Total: '+val.total_w_tax_prety+'</price>';
           htm+='</ons-col>';
...
function displayOrderDetails(data)
{
	...
	var header='<ons-col class="new_or">';
        header+='<ons-row>';
        header+='<p class="deatils_head_first">&nbsp;</p>';
		header+='<p class="deatils_head_second"><span class="big_bold">placed on:</span>  '+data.created_time+'</p>';
		header+='<br class="clear" />';
		header+='<p class="first_order_da"><span class="big_bold">Order No:</span> '+data.order_id+'</p>';
        header+='<p class="first_staust"> <span class="big_bold">Status:</span> '+data.status+'</p>';
        header+='</ons-row>';
     header+='</ons-col>';	 
...
/*	var html='';
	var html='<ons-list-header class="header">';
        html+='<ons-row>';
        html+='<ons-col class="first_staust"> Status: <ons-icon icon="'+icons.icons+'" class="icon '+icons.classname+'"></ons-icon>'+data.status+'</ons-col><ons-col class="first_order_da"> Order No: '+data.order_id+'</ons-col><br class="clear" />';
        html+='</ons-row>';
     ...
     html+='<ons-col  class="tab-bar_status">';
     html+='<p><ons-col> <span class="big_bold">costumer expects on:</span> '+data.del_time+'</ons-col></p>';
     html+='</ons-col>';	 
	...
	html+='<div class="height_scroll">';
     if (!empty(data.item)){
     	$.each( data.item , function( key, val ) {  
			  ...
			  final_price='';
			  description = val.category+ " : "+ val.item_name;
			  
			  description += '<div class="detail_price">'+val.total+'</div>';
     		  ...
     		  html+=TPLorderRowDetail( description ,  final_price , 'fixed-col bold' );     
     		  ...
     		  if (!empty(val.ingredients)){
     		  	  if ( val.ingredients.length>0){
	     		  	  ...
	     		  	  $.each( val.ingredients , function( key1, ingredients ) {       		  	  	  
	     		  	  	  html+=TPLorderRowDetail( ingredients ,  '' );     
	     		  	  });	
     		  	  }
     		  }
 		  	  	  ...
 		  	  	  $.each( val.sub_item_new , function( key2, sub_item ) {  
 		  	  	  	  ...
 		  	  	  	  if ( sub_item.length>0){
 		  	  	  	  	  $.each( sub_item , function( key3, sub_items ) {  
 		  	  	  	  	  	  ...
 		  	  	  	  	  	  html+=TPLorderRowDetail( t_desc ,  sub_items.total , 'fixed-col' );     
 		  	  	  	  	  });
 		  	  	  	  }
 		  	  	  });     		  	  
     		  }
...
/*     html+='<ons-list-header class="header">';
	...
	var option_time = data.option_time;
	option_time_arr = option_time.split(",");
		
		 
	html += '<div class="fixed_bottom"><p class="delivery_total_"><span class="big_bold">Total:</span> '+data.order_total_cost+'</p><div class="cookie_ss"><ul><li onclick="javascript:doSaveFeedback(\''+option_time_arr[0]+'\');" class="minu_1">'+option_time_arr[0]+'</li><li style="cursor:pointer;" onclick="javascript:doSaveFeedback(\''+option_time_arr[1]+'\');" class="minu_2">'+option_time_arr[1]+'</li><li style="cursor:pointer;" onclick="javascript:doSaveFeedback(\''+option_time_arr[2]+'\');" class="minu_3">'+option_time_arr[2]+'</li><li style="cursor:pointer;" onclick="javascript:doSaveFeedback(\''+option_time_arr[3]+'\');" class="minu_4">'+option_time_arr[3]+'</li><li style="cursor:pointer;" onclick="javascript:doSaveFeedback(\''+option_time_arr[4]+'\');" class="minu_5">'+option_time_arr[4]+'</li><li style="cursor:pointer;" onclick="javascript:doSaveFeedback(\''+option_time_arr[5]+'\');"  class="minu_6">'+option_time_arr[5]+'</li></ul><p id="pop" style="cursor:pointer;" onclick="javascript:openOtherForm(0);" class="delivery_total">other</p></div></div>';	 
...
function TPLorderRowDetail(label , value, label_class)
{

	 var html='';	
	 html+='<div class="new_list">';
       html+='<div class="ll">';
        html+='<div class="dfdf"'+label_class+'">'+label+'</div>';
        html+='<div class="del_value">'+value+'</div>';
		 html+='<br class="clear" />';
       html+='</div>';
     html+='</div>';
     return html	 
...
function openFeedbackForm(time_value)
{
	var order_id=$(".order_id").val();
	var trans_type=$(".trans_type").val();
	dump("order_id->"+order_id);
	...
	var options = {
      ...
      onTransitionEnd: function() {
		  ...
		  if( time_value == 0 ) { 
		     ...
		     doSaveFeedback(time_val);	
		  }
      } 
   };
   kNavigator.pushPage("saveFeedback.html", options);				
...
function openOtherForm()
{
	...
	var options = {
      ...
      onTransitionEnd: function() {
		  ...
		  params+="&mtid="+info.merchant_id;	 	 			 
		  params+="&order_id="+order_id;
		 callAjax("OtherList",params);	       
      } 
   };
   kNavigator.pushPage("saveOther.html", options);				
		
}

function openOutOfStockForm()
{
	var order_id=$(".order_id").val();
	var trans_type=$(".trans_type").val();
	dump("order_id->"+order_id);
	...
	var options = {
      ...
      onTransitionEnd: function() {
		  ...
		  params+="&mtid="+info.merchant_id;	 	 			 
		  params+="&order_id="+order_id;
		 callAjax("OutOfStock",params);	       
      } 
   };
   kNavigator.pushPage("outOfStock.html", options);				
...
{
	var htm='';
	htm+='<ons-list-header class="list-header">Provide Feedback</ons-list-header>';	
	htm+='<ons-list>';	
	htm+='</ons-list>';	
	
	createElement('feedback-list',htm);	
}


function doSaveFeedback(time_value)
{
	...
	$.validate({ 	
	    ...
	    onSuccess : function() {   
		  ...
		  if( time_value == 0 ) { 
	         var params = $( "#frm-savefeedback").serialize();
			 params+="&time_value="+time_value;
		  }else { 
		     var params ="time_value="+time_value;
		  }
	      ...
	      callAjax("DoSaveFeedback",params);	       
	      return false;
	    }  
	});	
...
function doSaveOtherFeedback(feedback)
{
		  var params ="remarks="+feedback;
		  params+="&time_value=0";
...
{
	var htm='';
	//htm+='<ons-list-header class="list-header">Select Feedback</ons-list-header>';	
	htm+='<ons-list>';	
	$.each( data.feedback_list, function( key, val ) {    

		ischecked='';

		htm+='<ons-list-item modifier="tappable">';
		 htm+='<label class="radio-button checkbox--list-item">';
			htm+='<div class="ora" onclick="javascript:doSaveOtherFeedback(\''+val+'\');">'+val+'</div>';
		  htm+='</label>'; 
		htm+='</ons-list-item>';
	});
	...
	htm+='<ons-list-item modifier="tappable">';
	 htm+='<label class="radio-button checkbox--list-item">';
		htm+='<div class="ora" onclick="javascript:openOutOfStockForm();">Out of stock</div>';
	  htm+='</label>'; 
	htm+='</ons-list-item>';	
	
	htm+='</ons-list>';	
	
	createElement('other-list',htm);	
}


function outOfStockList(data)
{
	var htm='';
	htm+='<ons-list-header class="list-header">Choose out of stock items</ons-list-header>';	
	htm+='<ons-list>';	
	$.each( data.status_list, function( key, val ) {    
		...
		htm+='<ons-list-item modifier="tappable">';
		 htm+='<label class="radio-button checkbox--list-item">';
			htm+='<input type="checkbox" name="items[]" class="status" value="'+val+'" >';
			htm+='<div class="radio-button__checkmark checkbox--list-item__checkmark"></div>';
			htm+=' '+val;
		  htm+='</label>'; 
		htm+='</ons-list-item>';
...
function saveOutOfStock()
{
	$.validate({ 	
	    ...
	    onSuccess : function() {     	   	      
	      var params = $( "#frm-outofstock").serialize();	      
	      ...
	      callAjax("DoSaveOutOfStockFeedback",params);	       
	      return false;
	    }  
	});	
}
...
jQuery(document).ready(function() {	
	...
	var check = $("#check_internal_timer").val();
	
	if( check == 1 ) {
		  //$("#p_incomming_orders").trigger("click");
		  
		  GetTodaysOrder1();
		  
	}else 	if( check == 2 ) {
		  //$("#p_in_progress_orders").trigger("click");
		  getPendingOrders1();
		  
	} else 	if( check == 3 ) {
		 // $("#p_completed_orders").trigger("click");
		  getGetAllOrders1();
	}
